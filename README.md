...
def get_users_all(db):
   """
    ...
    db: объект сессии.
    Что делает:
    1. Выполняет запрос к базе данных для поиска всех пользователей из таблицы User.
    """
...
#### Depends
>В контексте FastAPI и SQLAlchemy, Depends используется для управления зависимостями 
> — это мощный инструмент, который автоматически предоставляет объекты 
> (в данном случае, сессии базы данных) маршрутам или функциям. 
> Давайте разберём, зачем он нужен и как применяется с SQLAlchemy.

- Что делает Depends?
- - Автоматическое создание и управление зависимостями:

> С помощью Depends FastAPI автоматически вызывает функцию, переданную в Depends, 
> и предоставляет её результат маршруту или обработчику.
Например, Depends(get_db) вызывает функцию get_db, которая создаёт объект сессии базы данных.

- - Управление жизненным циклом объектов:

>Depends заботится о том, чтобы функция, создающая зависимость (например, get_db), 
> выполнилась корректно, а ресурсы (сессии, соединения с базой данных) 
> были освобождены в конце запроса.

- - Упрощение кода:

>Вместо передачи сессии базы данных вручную в каждый обработчик, 
> вы можете использовать Depends. Это делает код чище и избавляет от повторяющихся операций.
Пример использования Depends с SQLAlchemy
...
1. Создаёт сессию базы данных с помощью SessionLocal.

2. Гарантирует, что сессия будет закрыта после выполнения запроса, 
даже в случае ошибки (благодаря блоку finally).

>Пример маршрута с использованием Depends:
```python
...
FastAPI автоматически вызывает get_db через Depends, создавая объект сессии.
Этот объект (db) передаётся в обработчик маршрута get_users.
Сессия автоматически закрывается после завершения запроса.

>Зачем использовать Depends с SQLAlchemy?
1. Чистый и модульный код:
> Вы избегаете жёсткой привязки сессии базы данных ко всем маршрутам, у
> прощая тестирование и расширение.

2. Управление ресурсами:
>Depends заботится о корректном создании и закрытии сессии базы данных, 
> предотвращая утечки соединений.

3. Механизм повторного использования:
>get_db можно использовать повторно во всех маршрутах, что избавляет от дублирования кода.

4. Интеграция с FastAPI:
>Depends органично работает с другими функциями и зависимостями FastAPI.

- Итог
>Depends нужен для автоматизации передачи объектов (например, SQLAlchemy-сессий) в 
> функции маршрутов. Он упрощает управление зависимостями и делает код более лаконичным и надёжным.
